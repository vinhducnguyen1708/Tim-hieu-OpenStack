...
share_servers_handling_mode_opts = [
    ...
    cfg.StrOpt(
        "service_volume_type",
        default="__DEFAULT__",
        help="Name of volume type in Cinder, that will be used to create bootable volume"),
    cfg.StrOpt(
        "service_volume_size",
        default="10",
        help="Size in GB for volume bootable"),
    cfg.StrOpt(
...
class ServiceInstanceManager(object):
    ...
    def set_up_service_instance(self, context, network_info):
        ...
        # volume_instance = self._create_volume_from_image(
        #     context, instance_name)
        server = self._create_service_instance(
            context, instance_name, network_info, volume)
    ...
    def _create_volume_from_image(self, context, instance_name):
        """Creates volume bootable."""
        service_image_id = self._get_service_image(context)
        LOG.warning('This service instance will be create with bootable volume')
        service_volume_type = self.get_config_option("service_volume_type")
        service_volume_size = self.get_config_option("service_volume_size")
        create_kwargs = self._get_service_instance_create_kwargs()
        volumes = self.volume_api.create(
            context,
            name=instance_name,
            description='Manila share server bootable volume',
            size=self.get_config_option("service_volume_size"),
            volume_type=self.get_config_option("service_volume_type"),
            image_id=service_image_id,
            availability_zone=self.availability_zone,
            **create_kwargs)
        return volumes['id']
        
    def _create_service_instance(self, context, instance_name, network_info, volume_instance):
        """Creates service vm and sets up networking for it."""
        if not (self.get_config_option("service_volume_type") or self.get_config_option("service_volume_size")):
            service_image_id = self._get_service_image(context)
            block_device_mapping_v2 = None
        else:
            service_image_id = None
            volume_id = self._create_volume_from_image(context, instance_name)
            block_device_mapping_v2 = []
            bdm_dict = {'uuid':volume_id, 'source_type':'volume', 'destination_type':'volume', 'boot_index': 0, "delete_on_termination":"true"}
